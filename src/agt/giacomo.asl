   ...
   <- println;
    ...
    //  createGroup(hsh_group, house_group, GrArtId);
     // debug(inspector_gui(on))[artifact_id(GrArtId)];
      makeArtifact(bcart,"tools.BCArt",[],BCArtid)[wid(WOrg)];
      println("Blockchain artifact loaded");
      createGroupe(hsh_group, house_group, GrArtId);
      debug(inspector_gui(on))[artifact_id(GrArtId)];
      println("group created using blockchain artifact operation");
      println(GrArtId);
      adoptRolee(house_owner,GrArtId)[artifact_id(BCArtid)];
      focus(GrArtId);
